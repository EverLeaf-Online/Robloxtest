...
local function rememberLatestGrantedReceipt(player: Player, productId: number): boolean
	...
	local purchaseIds = data.Receipts.RecentPurchaseIds
	local purchaseId = purchaseIds[#purchaseIds]
	if typeof(purchaseId) ~= "string" or purchaseId == "" then
		return false
	end

	lastReceiptByPlayer[player] = {
...
local function testDeveloperProduct(player: Player, productName: string): (boolean, string)
	...
	-- ProductGranted normally captures this exact successful receipt. Keep a direct
	-- fallback for the Studio simulation path so replay is deterministic even if
	-- another listener changes later.
	lastReceiptByPlayer[player] = table.clone(receipt)
...
function StudioTestService.Init()
	...
	-- Capture every successfully granted developer-product receipt in Studio,
	-- including receipts generated by the real Marketplace purchase prompt.
	-- processReceipt records PurchaseId before ProductGranted fires, so the newest
	-- stored receipt ID is the exact receipt that was just granted.
	MonetizationService.ProductGranted:Connect(function(player, productName, productId)
